...
def get_model_handler_for_role(role: str, config: dict) -> ModelHandler | None:
    ...
    base_model_type = model_name.split('-')[0].lower() if '-' in model_name else model_name.lower()
    print(f"DEBUG [models/__init__.py]: For role '{role}', model_name='{model_name}', extracted base_model_type='{base_model_type}'")
