...
from core.models import get_model_handler_for_role, get_model_handler_by_identifier, GeminiHandler, ClaudeHandler, OpenAIHandler # Import specific handlers
...
DEFAULT_CONFIG = {
    ...
    'models': {
        'decomposition_model': 'gemini-2.0-flash',
        'consultation_models': ['gemini-2.0-flash', 'claude', 'openai'],
...
async def decompose_request(user_request: str, config: dict, decomposition_handler: Optional[ModelHandler], analyzer_engine: AnalyzerEngine) -> List[Dict[str, Any]]:
    """
...
"""
    ...
    try:
        ...
        if response['status'] == 'success' and response['content']:
            ...
            cleaned_json_string = analyzer_engine._clean_llm_json_output(raw_llm_output)
...
async def run_cli():
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            # Instantiate AnalyzerEngine WITH the handler and categories (STANDARD_CATEGORIES is internal to AnalyzerEngine now)
            analyzer_engine = AnalyzerEngine(config=config, analyzer_handler=analyzer_handler, standard_categories=None) # Pass None, AnalyzerEngine uses its own default
            ...
            decomposed_steps = await decompose_request(args.query, config, decomposition_handler, analyzer_engine)
            ...
            for step_info in decomposed_steps:
                    ...
                    for i, question_text in enumerate(step_questions):
                        ...
                        if not consultation_models_list:
                            ...
                            for model_idx, model_name in enumerate(consultation_models_list):
                                ...
                                try:
                                   # model_name from the loop is the specific model identifier (e.g., 'gemini-2.0-flash', 'claude-3-opus-20240229')
                                   # Use the new function to get handler by its specific identifier.
                                   # This function will derive base_model_type, get API key for base_model_type,
                                   # and instantiate the correct handler with the full model_name (identifier).
                                   consultation_handler = get_model_handler_by_identifier(model_name, config)
                                   # The specific_model_identifier variable is no longer needed here as model_name is the identifier.
                                   # The api_key_for_handler variable is also no longer needed here.
                                   # The conditional instantiation (if model_name == 'gemini': etc.) is handled by get_model_handler_by_identifier.

                                   if not consultation_handler or not consultation_handler.is_ready():
                                       # model_name here IS the specific identifier
                                       logger.warning(f"      Failed to initialize or ready handler for consultation model '{model_name}'. Skipping for question {current_question_number}.")
                                       continue

                                   # model_name here IS the specific identifier
                                   logger.info(f"      Consulting '{model_name}' (model {model_idx+1}/{len(consultation_models_list)}) for question {current_question_number}: '{question_text[:70]}...'")
                                   ...
                                   # model_name here IS the specific identifier
                                   logger.error(f"      Exception while trying to consult '{model_name}' for question {current_question_number} ('{question_text[:70]}...'): {e_model_consult}", exc_info=False)
                ...
                # analysis_result is now Dict[str, str] where keys are category_names and values are markdown strings.
                # Success is implied if it's a non-empty dict. AnalyzerEngine handles defaults for failed categories.
                if analysis_result and isinstance(analysis_result, dict) and any(analysis_result.values()): # Check if any markdown content was generated
                    logger.info(f"Markdown content generated for categories in substep {step_number}: {step_title}")
                    log_step(f"Markdown content generation successful for substep {step_number}", {"categories_generated": list(analysis_result.keys())})
                    
                    # Assemble the full markdown for this substep to be used as 'previous_substep_instructions'
                    # and to be passed to the synthesis engine.
                    assembled_markdown_for_substep_parts = []
                    # Import STANDARD_CATEGORIES from analyzer_engine to ensure consistent ordering for context passing
                    from core.analysis.engine import STANDARD_CATEGORIES as ORDERED_CATEGORIES_FOR_CONTEXT

                    for category_name_ctx in ORDERED_CATEGORIES_FOR_CONTEXT:
                        markdown_content_ctx = analysis_result.get(category_name_ctx)
                        if markdown_content_ctx and markdown_content_ctx.strip():
                             # For context, we might want a slightly different format than for the main doc,
                             # but task says "assembled Markdown content of the previously processed substep"
                             # which implies the same structure.
                            assembled_markdown_for_substep_parts.append(f"### {category_name_ctx}\n\n{markdown_content_ctx.strip()}\n")
                        else:
                            # Include a note if a category was expected but empty or had an error message.
                            # The default error message from AnalyzerEngine will be used if generation failed.
                             assembled_markdown_for_substep_parts.append(f"### {category_name_ctx}\n\n{analysis_result.get(category_name_ctx, '*No content.*')}\n")


                    # Include any non-standard categories (defensive, AnalyzerEngine should stick to its list)
                    for cat_name, md_val in analysis_result.items():
                        if cat_name not in ORDERED_CATEGORIES_FOR_CONTEXT:
                             assembled_markdown_for_substep_parts.append(f"### {cat_name} (Additional)\n\n{md_val.strip()}\n")
                    
                    previous_substep_instructions_str = "".join(assembled_markdown_for_substep_parts)
                    if not previous_substep_instructions_str.strip():
                        previous_substep_instructions_str = f"# No content generated for substep {step_title}."
                        logger.warning(f"Assembled previous_substep_instructions_str is empty for substep {step_title}.")
                    else:
                        logger.debug(f"Stored assembled Markdown for substep {step_number} as previous instructions context (length: {len(previous_substep_instructions_str)}). Preview: {previous_substep_instructions_str[:200]}...")

                    # Step 4: Append to Markdown (passing the dictionary of markdown strings)
                    if synthesis_engine and synthesis_engine.is_ready():
                        await synthesis_engine.append_substep_analysis_to_markdown(
                            output_filepath,
                            step_number,
                            step_title,
                            analysis_result # Pass the Dict[str, str] directly
                        )
                        log_step(f"Appended substep {step_number} Markdown to main document", {"output_file": output_filepath})
                    else:
                        logger.error(f"SynthesisEngine not ready. Cannot append Markdown for substep {step_number}.")
                        # If synthesis fails, previous_substep_instructions_str still holds the content.
                else:
                    # This case implies a more fundamental failure in AnalyzerEngine or it returned an empty dict.
                    # AnalyzerEngine is designed to return a dict with default error messages per category if LLM calls fail.
                    error_msg = "Markdown content generation failed or returned empty dictionary."
                    if analysis_result and 'error_message' in analysis_result: # Check for a potential top-level error from AnalyzerEngine (though not current design)
                         error_msg = analysis_result.get('error_message', error_msg)
                    
                    logger.error(f"Markdown content generation failed for substep {step_number}: {step_title}. Error: {error_msg}")
                    log_step(f"Markdown content generation failed for substep {step_number}", {"error": error_msg})
                    previous_substep_instructions_str = f"# Analysis for substep {step_title} failed catastrophically. Error: {error_msg}"
                    
                    # Append a generic error to the main document if synthesis engine is available
                    if synthesis_engine and synthesis_engine.is_ready():
                        await synthesis_engine.append_error_to_markdown(
                            output_filepath,
                            step_title,
                            f"Markdown content generation for this substep failed or was empty. Last error: {error_msg}"
                        )
                        log_step(f"Appended error for substep {step_number} to Markdown document", {"output_file": output_filepath})
                    else:
