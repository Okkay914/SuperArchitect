...
class GeminiHandler(ModelHandler):
    ...
    def __init__(self,
        ...
        self.model = model
        # If a generic "gemini" model key is passed, resolve it to a specific model identifier
        # This is to handle cases where the configuration might use "gemini" as a placeholder
        # or if older configurations are still using the generic term.
        if self.model == "gemini":
            logger.info(f"Generic model name '{model}' provided for GeminiHandler, resolving to 'gemini-2.0-flash' for API client initialization.")
            print(f"DEBUG [gemini.py]: Generic model name '{model}' provided, resolving to 'gemini-2.0-flash'.")
            self.model = "gemini-2.0-flash" # Use the specific, valid model name for the API client
